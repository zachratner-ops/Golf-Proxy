...
// Fetch DK + FD odds from the-odds-api.com
async function fetchGolfOdds() {
  try {
    ...
    const data = JSON.parse(body);
    // Build map: playerName -> { dk, fd }
    const odds = {};
    (data || []).forEach(event => {
      (event.bookmakers || []).forEach(bm => {
        const key = bm.key; // 'draftkings' or 'fanduel'
        (bm.markets || []).forEach(market => {
          (market.outcomes || []).forEach(outcome => {
            const name = outcome.name;
            const price = outcome.price; // American odds e.g. +1200
...
app.get('/golf/odds', async (req, res) => {
